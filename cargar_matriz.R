cargar_matriz<-function(ruta="Sudoku.xlsx")
{
  # Esta función carga el sudoku desde un Excel
  # Si no se pasa ningún argumento, carga por defecto el Excel "Sudoku.xlsx"
  ...
  # Argumentos:
  ## ruta: ruta del Excel de entrada para cargar. Por defecto es el Excel "Sudoku.xlsx" en la ruta del directorio
  ...
  archivo<-data.frame(read_excel(ruta,col_names=FALSE))
  ...
  if(rlen!=clen || sqrt(rlen)%%1 != 0)
  {
    ...
    archivo<-data.frame(read_excel(ruta,col_names=FALSE,range=rango_excel(maxlen)))
  }
